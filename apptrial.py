...
with features_and_output:
    ...
    with form:
        ...
        holiday_type = mid_col.radio("Holiday type:", options= sorted(set(data["holiday_type"])), horizontal= True)
...
#     categoric_columns = ['family', 'city', 'holiday_type']#, 'type_y', 'state']
...
#     input_data = np.concatenate([scaled_num, encoded_cat], axis=1)
...
def predict_sales(input_data, input_df):
    # Defining categories and numeric columns
    categoric_columns = ['family', 'city', 'holiday_type']
    columns = list(input_df.columns) 
    numeric_columns = [i for i in columns if i not in categoric_columns]
    
    # Scaling numeric columns and encoding categorical columns
    scaled_num = scaler.transform(input_df[numeric_columns])
    encoded_cat = encoder.transform(input_df[categoric_columns])
    
    # Convert input_data to a numpy array before concatenating
    input_data = np.concatenate([scaled_num, encoded_cat], axis=1)
